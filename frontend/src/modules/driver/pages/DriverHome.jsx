...
const DriverHome = () => {
        ...
        if (walletAlertState.isBlocked) {
            setStatusMessage(
                walletAlertState.belowMinimumBalance
                    ? `Wallet balance must be above Rs ${Number(walletAlertState.minimumBalanceForOrders || 0).toFixed(0)} to go online.`
    ...
    const handleDutyToggle = useCallback(() => {
        ...
        const requireDailySelfie = ['1', 'true', 'yes', 'on'].includes(
            String(settings?.customization?.enable_daily_driver_selfie ?? '0').trim().toLowerCase()
        );

        if (!requireDailySelfie || hasSelfieForToday(onlineSelfie)) {
            goOnline();
            return;
        }
        ...
        settings?.customization?.enable_daily_driver_selfie,
        stopSelfieCameraStream,
                    ...
                    if (nextWalletAlertState.isBlocked) {
                        ...
                        setStatusMessage(
                            nextWalletAlertState.belowMinimumBalance
                                ? `Wallet balance must be above Rs ${Number(nextWalletAlertState.minimumBalanceForOrders || 0).toFixed(0)} to receive new ride requests.`
