...
const ActiveTrip = () => {
    ...
    const completeRideAndExit = async () => {
            ...
            if (rideId) {
                await api.patch(
                    `/rides/${rideId}/status`,
                    {
                        ...
                        riderRating: selectedRating > 0 ? selectedRating : undefined,
                    },
                    withDriverAuthorization(driverToken),
                );
            }
        } catch {
            // The ride is very likely already completed by this point (payment
            // collection finalizes it a step earlier), so this call routinely
            // 409s here — that's expected, not a real failure to act on.
        }

        // The "rate the rider" star widget only appears after the ride is
        // already completed, so the rating needs its own dedicated call
        // rather than riding along with the (by-then-redundant) status PATCH.
        if (rideId && selectedRating > 0) {
            try {
                ...
                // Non-fatal — don't block the driver from exiting the trip over this.
            }
        }
    ...
    const resumeSimulation = () => {
        const pathExhausted = simulationStep >= simulationPathRef.current.length - 1;

        if (simulationPathRef.current.length < 2 || pathExhausted) {
            startSimulation();
            return;
        }
