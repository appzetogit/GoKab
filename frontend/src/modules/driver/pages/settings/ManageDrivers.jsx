...
import { getOwnerFleetDrivers, removeOwnerFleetDriver } from "../../services/registrationService";
...
const ManageDrivers = () => {
  ...
  // Only drop the row once the server confirms, otherwise the driver looks removed
  // but is still in the fleet and comes back on the next load.
  const deleteDriver = async (id) => {
    ...
    if (!window.confirm(`Remove ${driver?.name || "this driver"} from your fleet?`)) {
      return;
    }
    ...
    try {
      await removeOwnerFleetDriver(id);
      setDrivers((d) => d.filter((item) => item.id !== id));
    } catch (err) {
      setError(err?.message || "Could not remove this driver. Please try again.");
    } finally {
      setRemovingId("");
    }
  };
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        ...
                        className="p-2 bg-slate-50 rounded-lg text-slate-400 hover:text-rose-500 transition-colors disabled:opacity-40">
                        <Trash2 size={14} />
                      </button>
                    </div>
                  </div>
