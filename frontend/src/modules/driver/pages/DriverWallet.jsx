...
const transactionLabel = (tx = {}) => {
    ...
    if (type === 'adjustment') {
        ...
        const category = String(tx.metadata?.category || '').toLowerCase();
        const description = String(tx.description || '').toLowerCase();

        if (source === 'driver_referral' || description.includes('referral')) {
            return 'Referral reward';
        }
        if (category === 'driver_incentive') {
            return 'Incentive reward';
        }
        if (source === 'ride_cancellation_fee') {
...
const DriverWallet = () => {
    ...
    const [showTestRazorpayModal, setShowTestRazorpayModal] = useState(false);
    const [testOrderPayload, setTestOrderPayload] = useState(null);
    ...
    const handleConfirmTestRazorpayPayment = async () => {
        if (!testOrderPayload) return;
        setProcessingTopUp(true);
        setError('');
        try {
            const testPaymentId = `pay_test_${Date.now()}`;
            const verifyResponse = await api.post('/drivers/wallet/top-up/razorpay/verify', {
                orderId: testOrderPayload.orderId,
                paymentId: testPaymentId,
                signature: 'test_signature',
                razorpay_order_id: testOrderPayload.orderId,
                razorpay_payment_id: testPaymentId,
                razorpay_signature: 'test_signature',
                amount: testOrderPayload.amount,
            ...
            setShowTestRazorpayModal(false);
    ...
    const handleTopUp = async () => {
        ...
        try {
            ...
            if (orderData?.orderId?.startsWith('order_test_') || orderData?.keyId?.includes('DemoKey')) {
                setTestOrderPayload(orderData);
                setShowTestRazorpayModal(true);
                setProcessingTopUp(false);
                return;
            }
    ...
    return (
        <div className="min-h-screen bg-[#f5f1e8] px-4 pb-28 pt-4 text-slate-950">
            ...
            <AnimatePresence>
                {showTestRazorpayModal && (
                    <div className="fixed inset-0 z-[200] flex items-center justify-center bg-slate-950/75 p-4 backdrop-blur-md">
                        <Motion.div
                            initial={{ scale: 0.92, opacity: 0 }}
                            animate={{ scale: 1, opacity: 1 }}
                            exit={{ scale: 0.92, opacity: 0 }}
                            className="w-full max-w-sm overflow-hidden rounded-3xl bg-white shadow-2xl border border-slate-100"
                        >
                            {/* Razorpay Top Dark Bar */}
                            <div className="bg-[#0c2340] p-5 text-white">
                                <div className="flex items-center justify-between">
                                    <div className="flex items-center gap-2.5">
                                        <div className="flex h-8 w-8 items-center justify-center rounded-xl bg-[#20A354] font-black text-white text-xs shadow-md">
                                            RZP
                                        </div>
                                        <div>
                                            <p className="text-[10px] font-black text-emerald-400 uppercase tracking-widest">Razorpay Test Mode</p>
                                            <p className="text-sm font-extrabold text-white">{appName} Wallet Top-up</p>
                                        </div>
                                    </div>
                                    <button
                                        type="button"
                                ...
                                <div className="mt-4 rounded-2xl bg-white/10 p-3 text-center backdrop-blur-sm">
                                    <p className="text-[11px] font-semibold text-slate-300">Amount to Pay</p>
                                    <p className="text-3xl font-black text-emerald-400">{money(topUpAmount)}</p>
                                </div>
                            </div>

                            {/* Options Body */}
                            <div className="p-5 space-y-3">
                                <p className="text-xs font-black text-slate-400 uppercase tracking-wider">Select Payment Method (Test)</p>
                                
                                <div className="space-y-2">
                                    <div className="flex items-center justify-between rounded-2xl border-2 border-emerald-500 bg-emerald-50/50 p-3">
                                        <div className="flex items-center gap-3">
                                            <div className="grid h-8 w-8 place-items-center rounded-xl bg-emerald-500 text-white font-bold text-xs">UPI</div>
                                            <div>
                                                <p className="text-xs font-black text-slate-900">UPI / QR Code</p>
                                                <p className="text-[10px] text-slate-500">Google Pay, PhonePe, Paytm</p>
                                            </div>
                                        </div>
                                        <span className="h-3 w-3 rounded-full bg-emerald-500 ring-4 ring-emerald-200"></span>
                                    </div>

                                    <div className="flex items-center justify-between rounded-2xl border border-slate-100 bg-slate-50 p-3 opacity-80">
                                        <div className="flex items-center gap-3">
                                            <div className="grid h-8 w-8 place-items-center rounded-xl bg-slate-700 text-white font-bold text-xs">CARD</div>
                                            <div>
                                                <p className="text-xs font-bold text-slate-900">Cards (Visa / MasterCard)</p>
                                                <p className="text-[10px] text-slate-500">4111 •••• •••• 1111</p>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex items-center justify-between rounded-2xl border border-slate-100 bg-slate-50 p-3 opacity-80">
                                        <div className="flex items-center gap-3">
                                            <div className="grid h-8 w-8 place-items-center rounded-xl bg-slate-700 text-white font-bold text-xs">NET</div>
                                            <div>
                                                <p className="text-xs font-bold text-slate-900">Netbanking</p>
                                                <p className="text-[10px] text-slate-500">SBI, HDFC, ICICI, Axis</p>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="pt-2 space-y-2">
                                    <button
                                        type="button"
                                        onClick={handleConfirmTestRazorpayPayment}
                                        disabled={processingTopUp}
                                        className="flex h-12 w-full items-center justify-center gap-2 rounded-2xl bg-[#20A354] text-xs font-black uppercase tracking-wider text-white shadow-lg shadow-emerald-600/30 hover:bg-[#1b8c47] active:scale-98 transition-all"
                                    >
                                        {processingTopUp ? <RefreshCw className="animate-spin" size={16} /> : `Pay ${money(topUpAmount)} (Simulate Success)`}
                                    </button>
                                    <button
                                        type="button"
                                        onClick={() => {
                                            setShowTestRazorpayModal(false);
                                            setError('Payment failed or cancelled in test mode.');
                                        }}
                                        className="flex h-10 w-full items-center justify-center rounded-2xl bg-rose-50 text-xs font-bold text-rose-600 hover:bg-rose-100"
                                    >
