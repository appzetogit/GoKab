...
import {
    getDriverReferralProgram,
    getStoredDriverRegistrationSession,
    saveDriverReferral,
    saveDriverRegistrationSession,
} from '../../services/registrationService';

const describeRewardProgram = (program) => {
    if (!program?.enabled || !(program.amount > 0)) {
        return 'Ask your referrer about the current joining bonus terms.';
    }

    const amountText = `₹${program.amount} Bonus`;
    const isConditional = String(program.type || '').startsWith('conditional');

    if (isConditional && program.rideCount > 0) {
        return `Unlock ${amountText} after you complete your first ${program.rideCount} rides successfully.`;
    }

    return `Unlock ${amountText} credited to your wallet as soon as your referral code is applied.`;
};
...
const StepReferral = () => {
    ...
    useEffect(() => {
        ...
        getDriverReferralProgram()
            .then((response) => {
                if (!active) return;
                setRewardProgram(response?.data?.data || response?.data || null);
            })
            .catch(() => {});
                            ...
                            <div className="relative z-10 flex items-start gap-4">
                                ...
                                <div className="min-w-0 flex-1 space-y-1">
                                    ...
                                    <p className="text-[13px] text-slate-600 font-bold leading-relaxed opacity-80">
                                        {describeRewardProgram(rewardProgram)}
                                    </p>
                                </div>
                            </div>
