...
const RegistrationStatus = () => {
  ...
  useEffect(() => {
    if (location.state?.role) {
      const normalizedRole = normalizeDriverPortalRole(location.state.role) || "driver";
    ...
    if (onboardingToken) {
      const roleFromState = normalizeDriverPortalRole(location.state?.role);
      persistDriverAuthSession({
        token: onboardingToken,
        role: roleFromState || "driver",
      });
    }
...
const getStatusColor = (status) => {
  ...
  const isApplicationRejected = ['rejected', 'declined'].includes(String(driver?.status || '').toLowerCase());
  ...
  return (
    ...
    >
      <header className="mb-10 w-full max-w-sm flex items-center justify-between">
        {appLogo ? (
          <img
            src={appLogo}
            alt={appName}
        ...
        <button
          ...
          className="flex items-center gap-1.5 rounded-2xl bg-rose-50 border border-rose-100 px-3.5 py-2 text-[12px] font-black text-rose-600 shadow-sm active:scale-95 transition-all"
        >
        ...
        <section className="flex flex-col items-center text-center space-y-6">
            ...
            <div className="space-y-3">
                <p className="text-[11px] font-black uppercase tracking-[0.2em] text-slate-400 opacity-60">
                    {isApplicationRejected ? "Application Status" : pendingReverificationDocs.length > 0 ? "Submission received" : isVehicleReapproval ? "Update in review" : "Live Audit Status"}
                </p>
                <h1 className="font-['Poppins'] text-[42px] font-black leading-[1] tracking-[-0.04em] text-slate-900">
                    {isApplicationRejected ? (
                        <>Application <span className="text-slate-400">Rejected</span></>
                ...
                <p className="mx-auto max-w-[28ch] text-[15px] font-bold leading-relaxed text-slate-500 opacity-80">
                    {isApplicationRejected
                        ? "Your application was not approved. See the reason below, or contact support if you have questions."
                        : rejectedDocs.length > 0
                ...
                {isApplicationRejected && driver?.rejectionReason && (
                    <div className="mx-auto max-w-sm p-4 bg-rose-50 border border-rose-100 rounded-2xl text-left">
                        <p className="text-[13px] font-bold text-rose-600 leading-relaxed">
                            <span className="opacity-60 uppercase text-[10px] block mb-1 tracking-widest">Reason</span>
        ...
        {driver && (
            ...
            >
                <div className="flex items-center gap-4">
                    ...
                    <div className={`flex-shrink-0 px-3 py-1.5 rounded-full text-[9px] font-black uppercase tracking-[0.15em] shadow-lg border border-white/10 ${
                        isApplicationRejected ? 'bg-rose-600 text-white shadow-rose-600/20' : 'bg-slate-950 text-white shadow-slate-950/20'
                    }`}>
        ...
        <div className="sticky bottom-0 z-10 -mx-1 mt-2 rounded-[28px] border border-slate-100 bg-white/95 p-4 shadow-[0_-12px_30px_rgba(15,23,42,0.08)] backdrop-blur-xl flex flex-col gap-3">
            ...
            <div className="grid grid-cols-2 gap-3">
                <button 
                    ...
                    className="w-full h-14 bg-rose-50 border border-rose-100 text-rose-600 rounded-2xl flex items-center justify-center gap-2 text-[14px] font-bold active:scale-95 transition-all"
                >
