    ...
    <div className="min-h-screen bg-[#FBFBF6] font-sans text-gray-900">
      ...
      <div className="fixed top-0 left-0 right-0 bg-[#CFE8C9]/90 backdrop-blur-md z-50 border-b border-gray-200 shadow-sm">
        <div className="max-w-6xl mx-auto px-6 h-16 flex items-center gap-4">
            ...
            <span className="font-bold text-sm uppercase tracking-widest text-[#2F5F43]">Support Center</span>
        </div>
      </div>

      {/* Hero Section */}
      <div className="bg-[#2F5F43] text-white pt-32 pb-24 px-6">
        <div className="max-w-4xl mx-auto text-center">
            <h1 className="text-5xl md:text-6xl font-black tracking-tight mb-6">
                Frequently Asked <span className="text-[#F2D34F]">Questions</span>
            </h1>
      ...
      <div className="max-w-4xl mx-auto px-6 py-16 -mt-10 relative z-10">
        <div className="bg-[#CFE8C9] rounded-3xl shadow-xl p-6 md:p-10 border border-gray-100">
            <div className="space-y-4">
                {faqs.map((faq, index) => (
                    <div 
                        key={index} 
                        className={`border rounded-2xl overflow-hidden transition-all duration-300 ${openIndex === index ? 'border-[#F2D34F] bg-[#FBFBF6]/50' : 'border-gray-100 bg-[#FBFBF6] hover:border-gray-300'}`}
                    >
                        <button 
                            className="w-full text-left px-6 py-5 flex items-center justify-between gap-4"
                            onClick={() => setOpenIndex(openIndex === index ? -1 : index)}
                        >
                            <h3 className={`font-bold text-lg pr-8 ${openIndex === index ? 'text-[#2F5F43]' : 'text-gray-700'}`}>{faq.q}</h3>
                            <ChevronDown className={`flex-shrink-0 transition-transform duration-300 ${openIndex === index ? 'rotate-180 text-[#F2D34F]' : 'text-gray-400'}`} size={20} />
                        </button>
