...
const quickCards = [
  {
    ...
    helper: 'Call for booking, ride, or account help',
    Icon: Phone,
  },
  {
...
const SupportPage = () => {
  ...
  return (
    <div className="min-h-screen bg-[#FBFBF6] text-slate-900">
      <div className="fixed left-0 right-0 top-0 z-50 border-b border-stone-200 bg-[#CFE8C9]/90 backdrop-blur-md">
      ...
      <section className="bg-[#2F5F43] px-6 pb-16 pt-28 text-white">
        <div className="mx-auto max-w-6xl">
          ...
          <h1 className="max-w-4xl text-4xl font-black tracking-tight md:text-6xl">
            Talk to <span className="text-[#F2D34F]">Support</span>
          </h1>
          <p className="mt-6 max-w-3xl text-lg leading-relaxed text-stone-300">
            Reach the client support team for ride issues, payment questions, account updates,
          ...
          <div className="mt-8 grid gap-4 md:grid-cols-2">
            {quickCards.map((item) => (
              ...
              >
                <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-white/5 text-[#F2D34F]">
                  <item.Icon size={22} />
                </div>
      ...
      <section className="px-6 py-16">
        <div className="mx-auto grid max-w-6xl gap-6 md:grid-cols-3">
          {detailCards.map((card) => (
            <div
              key={card.title}
              className="rounded-[28px] border border-stone-200 bg-[#CFE8C9] p-7 shadow-sm"
            >
              <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-[#FBFBF6] text-[#2F5F43]">
                <card.Icon size={22} />
              </div>
              <p className="mt-5 text-[11px] font-black uppercase tracking-[0.24em] text-stone-400">
                {card.title}
              </p>
              <p className="mt-2 text-xl font-black leading-tight text-[#2F5F43]">
                {card.value}
              </p>
