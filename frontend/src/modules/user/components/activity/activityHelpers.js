...
export const TABS = ['All', 'Rides', 'Rental', 'Pooling', 'Outstation', 'Scheduled', 'Support'];
...
export const getRideTimeSource = (ride) => {
  // A scheduled trip that hasn't started yet is about its *pickup* time — showing
  // `createdAt` there surfaces when the user tapped Book, which reads as a wrong
  // date on the card. Once the trip actually runs, the real event stamps win.
  if (ride?.scheduledAt && !ride?.startedAt && !ride?.completedAt) {
    return ride.scheduledAt;
  }

  return ride.completedAt || ride.startedAt || ride.acceptedAt || ride.createdAt || ride.updatedAt;
};
...
export const normalizeRide = (ride) => {
  ...
  // Stays empty while the ride is still unassigned — a placeholder name here
  // leaks into the card title as "Scheduled ride with Driver assigned".
  ...
  const withDriver = driverName ? ` with ${driverName}` : '';
  // The card still needs something to label the avatar with while unassigned.
  const driverDisplayName = driverName || 'Driver to be assigned';
  ...
  const title = type === 'parcel'
    ...
    : isScheduled
      ? `Scheduled ride${withDriver}`
      : isOutstation
        ? `Outstation trip${withDriver}`
        : (status === 'Searching' ? 'Ride request' : `Ride${withDriver}`);
  ...
  return {
    ...
    driverImage: pickFirstString(
      ...
      ride?.driver?.image,
      ride?.driver?.avatar,
      buildAvatarFallback(driverDisplayName),
    ),
