...
const RentalDeposit = () => {
  ...
  const handlePay = async () => {
    ...
    try {
      ...
      const isTestModeOrder = order.orderId.startsWith('order_test_')
        || String(order.keyId || '').includes('DemoKey')
        || String(order.keyId || '').includes('demo');

      if (isTestModeOrder) {
        setTestOrderPayload({ order, methodLabel });
  ...
  const handleConfirmTestPayment = async () => {
    ...
    const { order, methodLabel } = testOrderPayload;
    setPaying(true);
    try {
      const verifyResponse = await userService.verifyRentalAdvancePayment({
      ...
      {showTestPaymentModal ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/50 px-6">
          <div className="w-full max-w-sm rounded-[24px] bg-white p-6 shadow-2xl">
            <h3 className="text-lg font-black text-slate-900">Razorpay Test Mode</h3>
            <p className="mt-1 text-[13px] font-medium text-slate-500">
              No live payment gateway is configured. Simulate a successful payment to continue.
            </p>
            <div className="mt-4 rounded-[14px] bg-slate-50 px-4 py-3 text-center">
              <span className="text-2xl font-black text-slate-900">Rs.{payableNow}</span>
            </div>
            {paymentError ? (
              <div className="mt-3 rounded-[14px] border border-rose-100 bg-rose-50 px-3 py-2 text-[11px] font-bold text-rose-500">
                {paymentError}
              </div>
            ) : null}
            <button
              ...
              disabled={paying}
              className="mt-4 w-full rounded-[16px] bg-slate-950 py-3.5 text-[14px] font-bold text-white flex items-center justify-center gap-2"
            >
              {paying ? (
                <span className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />
              ) : (
                `Pay Rs.${payableNow} (Simulate Success)`
              )}
            </button>
            <button
              type="button"
              ...
              disabled={paying}
              className="mt-2 w-full rounded-[16px] py-3 text-[13px] font-bold text-slate-400"
            >
              Cancel
            </button>
          </div>
