...
const PoolingVehicleForm = ({
  ...
  const [formData, setFormData] = useState({
    ...
    ...initialFormData,
    blueprint: initialFormData?.blueprint || {
      rows: 3,
      cols: 2,
      layout: [],
    },
  ...
  useEffect(() => {
    if (id && !service) {
      loadVehicle();
    } else if (!formData.blueprint?.layout?.length) {
      // Set default layout for the initial vehicle type
      setFormData(prev => ({
        ...prev,
        blueprint: generateDefaultLayout(prev.vehicleType || 'sedan')
      }));
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (service) {
        const response = await service.createPoolingVehicle(payload);
        toast.success(createSuccessMessage);
        if (onSaveSuccess) {
          await onSaveSuccess(response, payload);
        }
        return;
      }
      ...
      <div className="mx-auto max-w-5xl">
          ...
          <div className="flex items-center justify-between">
            ...
            {isViewMode ? (
              ...
              >
                ...
                {createActionLabel || (isEditMode ? 'Save Changes' : 'Create Vehicle')}
              </button>
            ) : null}
          </div>

          {helperPanel ? (
            <div className="mt-6 rounded-2xl border border-indigo-100 bg-indigo-50 p-4 text-sm font-semibold leading-6 text-indigo-900">
              {helperPanel}
            </div>
          ) : null}
        </div>
        ...
        <div className="grid gap-4 md:gap-8 lg:grid-cols-5">
          ...
          <div className="lg:col-span-2 space-y-6">
            {(formData.driverName !== undefined || formData.driverPhone !== undefined) ? (
              <div className="rounded-[32px] border border-slate-100 bg-white p-4 md:p-8 shadow-sm">
                <h3 className="mb-6 text-lg font-black text-slate-900">Driver Details</h3>
                        ...
                        reader.onloadend = async () => {
                          ...
                          try {
                            const res = service
                              ...
                              : await adminService.uploadImage(base64);
                            setFormData(prev => ({
                              ...prev,
        ...
        {placeCreateActionAtEnd && !isViewMode ? (
