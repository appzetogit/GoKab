...
const ImageUploadBox = ({ title, infoText, preview, onUpload, onClear }) => {
  const fileInputRef = useRef(null);
  return (
    <div className="space-y-3">
       <div className="flex items-center justify-between px-0.5">
          <label className="text-xs font-bold text-gray-700">{title}</label>
          <span className="text-[10px] font-bold text-indigo-600 bg-indigo-50 px-2 py-0.5 rounded-md">{infoText || "Any size accepted • Auto-optimized"}</span>
       </div>
       <div className="aspect-[2/1] w-full rounded-2xl border-2 border-dashed border-gray-200 bg-gray-50/50 relative overflow-hidden group hover:border-indigo-400 transition-colors cursor-pointer flex items-center justify-center shadow-inner" onClick={() => fileInputRef.current?.click()}>
          {preview ? (
            <img src={preview} alt={title} className="max-w-full max-h-full object-contain p-4 transition-transform duration-300 group-hover:scale-105" />
          ) : (
            <div className="flex flex-col items-center justify-center gap-2 text-gray-400">
                <div className="w-12 h-12 rounded-2xl bg-white flex items-center justify-center text-indigo-500 shadow-sm border border-gray-100">
                  <Upload size={22} strokeWidth={2} />
                </div>
                <p className="text-[11px] font-bold uppercase tracking-wider text-gray-500">Upload {title}</p>
                <span className="text-[10px] text-gray-400 font-medium">Supports PNG, JPG, WEBP, SVG (Auto-scaled)</span>
            </div>
          )}
          
          <div className="absolute top-4 right-4 flex items-center gap-2">
             <button onClick={(e) => { e.stopPropagation(); fileInputRef.current?.click(); }} className="w-8 h-8 rounded-xl bg-white text-indigo-600 shadow-md border border-gray-100 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                <Upload size={14} />
             </button>
             {preview && (
               <button onClick={(e) => { e.stopPropagation(); onClear(); }} className="w-8 h-8 rounded-xl bg-rose-50 text-rose-600 shadow-md border border-rose-100 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                  <X size={14} />
               </button>
             )}
          </div>
          <input type="file" accept="image/*" className="hidden" ref={fileInputRef} onChange={(e) => { if(e.target.files[0]) onUpload(e.target.files[0]); }} />
       </div>
    </div>
  );
};
...
const autoOptimizeLogoDataUrl = (dataUrl) =>
  new Promise((resolve) => {
...
const autoOptimizeFaviconDataUrl = (dataUrl, targetSize = 64) =>
  new Promise((resolve) => {
    const image = new Image();
    image.onload = () => {
      const canvas = document.createElement('canvas');
...
const GeneralSettings = () => {
  ...
  const handleLogoUpload = async (file) => {
    try {
      ...
      const optimizedLogo = await autoOptimizeLogoDataUrl(dataUrl);
      handleChange('general', 'logo', optimizedLogo);
      toast.success('Logo uploaded and auto-optimized!');
    } catch (err) {
  ...
  const handleFaviconUpload = async (file) => {
    try {
      const dataUrl = await fileToDataUrl(file);
      const optimizedFavicon = await autoOptimizeFaviconDataUrl(dataUrl, 64);
      handleChange('general', 'favicon', optimizedFavicon);
      setLiveFavicon(optimizedFavicon);
      toast.success('Favicon uploaded and auto-scaled!');
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4 md:p-6 lg:p-10 font-sans">
      ...
      <div className="max-w-[1600px] mx-auto space-y-10 pb-32">
        ...
        {/* Media Assets */}
        <SectionCard title="Brand Assets & Icons">
           <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
              <ImageUploadBox
                title="Brand Logo"
                infoText="Any size / format • Auto-scaled"
              ...
              <ImageUploadBox
                ...
                infoText="Any size / format • Auto-fitted"
