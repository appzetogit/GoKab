...
export const resolveConfiguredGatewayCredentials = async (gatewayKey) => {
  ...
  if (gatewayKey === 'razor_pay') {
    ...
    if (keyId.toLowerCase().includes('demo') || keySecret.toLowerCase().includes('demo') || !keyId) {
      if (isLive) {
        throw new ApiError(500, 'Razorpay keys are demo placeholders. Configure real live keys in Admin > Payment Gateways');
      }
      keyId = 'rzp_test_GoKabDemoKey';
      keySecret = 'GoKabDemoSecret';
    }
  ...
  if (gatewayKey === 'phone_pay') {
    let clientId = getFirstConfiguredValue(validatedGateway, PHONEPE_FIELD_ALIASES.merchant_id);
    let clientSecret = getFirstConfiguredValue(validatedGateway, PHONEPE_FIELD_ALIASES.salt_key);
    const clientVersion = getFirstConfiguredValue(validatedGateway, PHONEPE_FIELD_ALIASES.salt_index) || '1';

    if (clientId.toLowerCase().includes('demo') || clientSecret.toLowerCase().includes('demo') || !clientId) {
      if (isLive) {
        throw new ApiError(500, 'PhonePe keys are demo placeholders. Configure real live keys in Admin > Payment Gateways');
      }
      clientId = 'PGTESTPAYUAT';
      clientSecret = '099eb0cd-02aa-4e41-b180-2bfa370944e5';
    }
