...
// A cancelled trip was never taken, so the rider must get the promo use back —
// otherwise booking and cancelling silently burns their allowance and eats into
// their cumulative discount ceiling for a discount they never received.
// Idempotent: the redemption row flips to `cancelled` and is only reversed once,
// so repeated cancellations of the same ride cannot refund the use twice.
export const releasePromoForCancelledRide = async ({ ride, session = null } = {}) => {
  const promoId = ride?.promo?.promo_id;
  const rideId = ride?._id;

  if (!promoId || !rideId) {
    return { released: false, reason: 'no-promo' };
  }

  const redemption = await PromoRedemption.findOneAndUpdate(
    { promo_id: promoId, ride_id: rideId, status: 'applied' },
    { $set: { status: 'cancelled' } },
    { returnDocument: 'after', ...(session ? { session } : {}) },
  );

  // Already released (or never applied) — nothing to give back.
  if (!redemption) {
  ...
  await PromoCode.updateOne(
    { _id: promoId, usage_count: { $gt: 0 } },
    { $inc: { usage_count: -1 } },
    session ? { session } : {},
  );

  await PromoUserCounter.updateOne(
    { promo_id: promoId, user_id: redemption.user_id, uses_count: { $gt: 0 } },
    { $inc: { uses_count: -1, cumulative_discount_amount: -discountAmount } },
    session ? { session } : {},
  );

  return { released: true, discountAmount };
};
