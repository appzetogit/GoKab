...
const SUPPORT_ROLE_RE = /^(admin|user|driver|owner)$/;
const SUPPORT_CONVERSATION_CANONICAL_RE = /^(user|driver|owner):([^:]+):([^:]+)$/;
const SUPPORT_CONVERSATION_LEGACY_RE = /^admin:([^|]+)\|(user|driver|owner):([^|]+)$/;
...
export const resolveSupportConversationKey = async ({
  ...
  if (normalizedSenderRole !== 'user' && normalizedSenderRole !== 'driver' && normalizedSenderRole !== 'owner') {
