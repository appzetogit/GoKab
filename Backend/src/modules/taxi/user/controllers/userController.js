...
import { assertNoOutstandingUserDues, emitToDriver } from '../../services/dispatchService.js';
...
const razorpayRequest = async ({ method, path, body, keyId, keySecret }) => {
  if (keyId?.includes('DemoKey') || keyId?.includes('demo')) {
    const mockId = path.startsWith('/orders')
      ? `order_test_${Date.now().toString(36)}`
      : `rfnd_test_${Date.now().toString(36)}`;

    return {
      id: mockId,
      entity: path.startsWith('/orders') ? 'order' : 'refund',
      amount: body?.amount || 0,
      currency: body?.currency || 'INR',
      status: path.startsWith('/orders') ? 'created' : 'processed',
...
const verifyAndApplyUserRazorpayWalletTopup = async ({
  ...
  const isTestModePayment =
    normalizedOrderId.startsWith('order_test_') || normalizedPaymentId.startsWith('pay_test_');
  ...
  if (isTestModePayment) {
    ...
    // showed the user) instead of fetching a non-existent real order.
    amountPaise = Number(requestedAmountPaise) > 0 ? Math.round(Number(requestedAmountPaise)) : 0;
    ...
    orderUserId = String(order?.notes?.userId || '').trim();
  }
...
const verifyRazorpayRentalAdvanceGatewayPayment = async ({
  ...
  const isTestModePayment = normalizedOrderId.startsWith('order_test_') || normalizedPaymentId.startsWith('pay_test_');
...
export const createBusBookingOrder = async (req, res) => {
  ...
  // Gated before any seat hold or payment order is created.
...
export const verifyBusBookingPayment = async (req, res) => {
  ...
  const isTestModePayment = orderId.startsWith('order_test_') || paymentId.startsWith('pay_test_');
...
export const createRentalBookingRequest = async (req, res) => {
  // Gated before the booking request is raised or any advance is taken.
  await assertNoOutstandingUserDues(req.auth?.sub);
