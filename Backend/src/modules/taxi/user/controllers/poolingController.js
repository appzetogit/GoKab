...
import { assertNoOutstandingUserDues } from '../../services/dispatchService.js';
...
export const createPoolingBookingOrder = asyncHandler(async (req, res) => {
  ...
  // Gated before any seat is reserved or payment order is created.
  ...
  const amountPaise = Math.round(fareBreakdown.totalFare * 100);
  const order = (keyId?.includes('DemoKey') || keyId?.includes('demo'))
    ? {
