...
export const pushBanner = async (id) => {
  ...
  // This used to only bump the counter and report success, so the banner was
  // never actually pushed to anyone while the admin was told it had been.
  const delivery = await sendPushNotificationToAudience({
    serviceLocationId: null,
    sendTo: 'users',
    title: banner.title || 'New offer',
    body: banner.title ? '' : 'Tap to see the latest offer',
    image: typeof banner.image === 'string' ? banner.image : banner.image?.url || '',
  });

  // Only count a push that actually went out, otherwise the counter overstates reach.
  ...
  return {
    message: delivery.attempted
      ...
      : delivery.reason || 'Banner saved, but push delivery is not configured',
    delivery,
