...
const DRIVER_VEHICLE_FIELD_DEFINITIONS = {
  ...
  taxNumber: { label: 'Tax Number (GST/VAT)', field_type: 'text', field_group: 'owner', account_type: 'fleet_drivers', sort_order: 70, placeholder: 'Enter GST/VAT Number (Optional)', help_text: 'Optional. You can provide your GST/VAT number now or update it later from your profile.' },
};
...
const serializeDriverListItem = (driver) => ({
  ...
  driver_code: driver.referralCode || (driver.phone ? `DRV${String(driver.phone).slice(-4)}${String(driver._id || '').slice(-6).toUpperCase()}`.replace(/\W/g, '') : ''),
  referralCode: driver.referralCode || '',
  account_type: driver.account_type || driver.onboarding?.role || 'individual',
  // Driver-network category is separate from `account_type`, which is derived
  // from vehicle count by the reconciliation cron.
  driver_category: driver.driver_category || 'lower',
  vehicle_usage_type: driver.vehicle_usage_type || '',
  route_mode: driver.route_mode || 'all_locations',
  wallet_balance: Number(driver.wallet?.balance || 0),
  wallet_frozen: Number(driver.wallet?.frozenBalance || 0),
  wallet_available:
    Math.round((Number(driver.wallet?.balance || 0) - Number(driver.wallet?.frozenBalance || 0)) * 100) / 100,
  onboarding_role: driver.onboarding?.role || driver.account_type || '',
  owner_id: driver.owner_id || null,
  service_location_id: driver.service_location_id || null,
  city: driver.city || '',
...
export const forgotPassword = async (email) => {
  ...
  await sendEmail({
    to: email,
    subject: `Password Reset OTP for ${process.env.APP_NAME || 'GoKab'}`,
    ...
    html: `
      <div style="font-family: sans-serif; padding: 20px; border: 1px solid #eee; border-radius: 10px; max-width: 500px;">
        ...
        <p style="font-size: 12px; color: #666;">Regards,<br>Team ${process.env.APP_NAME || 'GoKab'}</p>
      </div>
    `,
  });
...
export const getDriverRatingDetail = async (id) => {
  ...
  // Only rides the rider actually left feedback on count as a "review" —
  // otherwise every ride (rated or not) would show up in the list.
  ...
  return {
    ...
    // Each review must show that specific ride's own rating/comment, not the
    // driver's overall average repeated for every row.
    reviews: rides.map((ride) => ({
      _id: ride._id,
      request_id: String(ride._id),
      date: ride.createdAt,
...
export const adjustDriverWallet = async (id, payload = {}) => {
  ...
  try {
    ...
    // Recompute against the same wallet rules (minimum-balance-for-orders,
    // wallet-enabled toggle, owner-managed exemption) used everywhere else,
    // instead of an ad-hoc cashLimit-only check that could leave a driver
    // wrongly marked eligible for dispatch after this adjustment.
    const wallet = await getWalletSnapshot(driver);
    const cashLimit = wallet.cashLimit;
    const isBlockedAfter = wallet.isOwnerManagedDriver
      ? false
      : (!wallet.rules.isWalletEnabled || wallet.balance <= wallet.minimumBalanceForOrders);
...
// Fleet-wide wallet summary plus a recent transaction ledger. The admin driver
// wallet screen previously rendered hardcoded figures, so it reported a balance
// pool and transactions that existed nowhere in the data.
export const getDriverWalletLedger = async (query = {}) => {
  ...
  const [balanceAgg, pendingAgg, processedTodayAgg, failedCount] = await Promise.all([
    Driver.aggregate([
      { $match: { deletedAt: null } },
      { $group: { _id: null, total: { $sum: { $ifNull: ['$wallet.balance', 0] } } } },
    ]),
    WithdrawalRequest.aggregate([
    ...
    WithdrawalRequest.countDocuments({ status: 'cancelled' }),
  ]);

  let driverFilter = null;
  if (search) {
    const regex = new RegExp(search.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'i');
    const matched = await Driver.find({ $or: [{ name: regex }, { phone: regex }] }).select('_id').lean();
    driverFilter = matched.map((d) => d._id);
  }

  const txQuery = driverFilter ? { driverId: { $in: driverFilter } } : {};
  ...
  return {
    stats: {
      ...
      processedToday: Math.round((processedTodayAgg[0]?.total || 0) * 100) / 100,
      failedTransactions: failedCount,
    },
    results: transactions.map((t) => ({
      _id: String(t._id),
      driver: t.driverId?.name || 'Unknown driver',
      driverPhone: t.driverId?.phone || '',
      amount: Number(t.amount || 0),
      type: Number(t.amount || 0) < 0 ? 'Debit' : 'Credit',
      category: t.type || 'adjustment',
...
export const adjustOwnerWallet = async (id, payload = {}) => {
  ...
  try {
    ...
    await session.withTransaction(async () => {
      const owner = await Owner.findById(id).session(session);
      if (!owner) {
        throw new ApiError(404, 'Owner not found');
      }

      const currentBalance = Number(owner.wallet?.balance || 0);
      nextBalance = operation === 'credit' ? currentBalance + amount : currentBalance - amount;
      ...
      const created = await OwnerWalletTransaction.create(
        [{
          ownerId: id,
          amount,
          kind: operation,
    ...
    return { balance: Number(nextBalance.toFixed(2)), transaction };
  } finally {
...
export const updateDriver = async (id, payload, currentAdmin = null) => {
  ...
  const rejectionReasonValue = payload.rejection_reason ?? payload.rejectionReason;
  if (rejectionReasonValue !== undefined) {
    update.rejectionReason = String(rejectionReasonValue || '').trim();
  } else if (update.status && update.status !== 'rejected') {
    // Clear any stale rejection reason once the application moves off the rejected state.
...
export const getSubscriptionSettings = async () => {
  ...
  const mode = setting?.subscription?.mode || setting?.driver_subscriptions?.mode || 'both';
...
export const updateSubscriptionSettings = async (payload) => {
  ...
  const setting = await AdminBusinessSetting.findOneAndUpdate(
    { scope: 'default' },
    { $set: { 'subscription.mode': mode, 'driver_subscriptions.mode': mode } },
    { new: true, upsert: true },
  ).lean();

  return { success: true, mode: setting?.subscription?.mode || mode };
};
...
const toAdminRideRow = (ride) => {
  ...
  return {
    ...
    date: ride.createdAt,
    userName: ride.userId?.name || ride.offline_customer?.name || 'Unknown User',
...
const toAdminDeliveryRow = (ride) => {
  ...
  return {
    ...
    requestId: requestCode,
    date: ride.createdAt,
    userName: ride.userId?.name || ride.offline_customer?.name || parcel.senderName || 'Unknown User',
    driverName: ride.driverId?.name || 'Unassigned',
...
export const listRideRequests = async (query = {}) => {
  ...
  if (query.userId || query.user_id) {
    filter.userId = query.userId || query.user_id;
  }
  if (query.driverId || query.driver_id) {
    filter.driverId = query.driverId || query.driver_id;
  }
  ...
  if (query.driver_search) {
    const ds = String(query.driver_search).toLowerCase();
    rows = rows.filter((row) => String(row.driverName || '').toLowerCase().includes(ds) || String(row.driverPhone || '').toLowerCase().includes(ds));
  }

  if (query.user_search) {
    const us = String(query.user_search).toLowerCase();
    rows = rows.filter((row) => String(row.userName || '').toLowerCase().includes(us) || String(row.userPhone || '').toLowerCase().includes(us));
  }
...
export const listOwnerDocumentUploadFields = async ({ activeOnly = true } = {}) => {
  ...
  export const buildDriverReport = async (query = {}) => {
    const { transport_type, vehicle_type, status, approval_status, date_option, from_date, to_date } = query;
    const filter = {};
    ...
    if (vehicle_type) {
      // The picker lists catalog vehicle types by name ("GoKab SUV"), while a driver
      // stores the coarse `vehicleType` enum plus a `vehicleTypeId` pointing at that
      // catalog entry. Matching the name straight against `vehicleType` silently
      // returned an empty report, so resolve the catalog entry and match either side.
      const catalogEntry = await Vehicle.findOne({
        $or: [
          ...(mongoose.isValidObjectId(vehicle_type) ? [{ _id: vehicle_type }] : []),
          { name: new RegExp(`^${String(vehicle_type).replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}$`, 'i') },
        ],
      }).select('_id name icon_types').lean();

      if (catalogEntry) {
        // Match the catalog link only. `icon_types` is shared across entries
        // (several are "car"), so matching it would return every driver.
        filter.vehicleTypeId = catalogEntry._id;
      } else {
        // Unknown value: treat it as a legacy free-text vehicle label.
        const rx = new RegExp(`^${String(vehicle_type).replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}$`, 'i');
        filter.$or = [{ vehicleType: rx }, { vehicleIconType: rx }];
      }
    }

    // The picker sends `approval_status`; reading only `status` meant every
    // "pending drivers" export silently contained approved drivers too.
    const effectiveApprovalStatus = String(approval_status || status || '').trim().toLowerCase();
    if (effectiveApprovalStatus) {
      filter.status = effectiveApprovalStatus === 'disapproved' ? 'rejected' : effectiveApprovalStatus;
    }
...
export const buildDriverDutyReport = async (query = {}) => {
  ...
  export const buildOwnerReport = async (query = {}) => {
    const { service_location_id, status, approval_status, date_option, from_date, to_date } = query;
    const filter = {};

    if (service_location_id && mongoose.isValidObjectId(service_location_id)) {
      filter.service_location_id = toObjectId(service_location_id);
    }

    // The picker sends `approval_status` (approved / pending / blocked).
    const effectiveStatus = String(approval_status || status || '').trim().toLowerCase();
    if (effectiveStatus === 'approved' || effectiveStatus === 'active') {
      filter.active = true;
    } else if (effectiveStatus === 'pending') {
      filter.status = 'pending';
    } else if (effectiveStatus === 'blocked' || effectiveStatus === 'inactive') {
      filter.active = false;
    }
    ...
    // `listOwners` builds its own query and ignores anything passed in, so routing
    // this report through it dropped every filter — the export was always the full
    // owner list regardless of location, status or date. Query directly instead.
    await ensureFleetOwnersSeeded();
  ...
  // Took no arguments at all, so every filter the screen offers (fleet, trip status,
  // date range) was discarded — and despite the name it returned no financial
  // figures, just an owner roster. Now scoped and aggregated per fleet owner.
  export const buildFleetFinanceReport = async (query = {}) => {
    const { fleet_id, trip_status, status, date_option, from_date, to_date } = query;

    // The picker lists service locations, so `fleet_id` scopes owners by location.
    const ownerFilter = {};
    if (fleet_id && mongoose.isValidObjectId(fleet_id)) {
      ownerFilter.service_location_id = toObjectId(fleet_id);
    }

    await ensureFleetOwnersSeeded();
    const owners = await Owner.find(ownerFilter).sort({ createdAt: -1 }).lean();

    const drivers = await Driver.find({ owner_id: { $in: owners.map((o) => o._id) } })
      .select('_id owner_id')
      .lean();
    const ownerByDriver = new Map(drivers.map((d) => [String(d._id), String(d.owner_id)]));

    const rideFilter = { driverId: { $in: drivers.map((d) => d._id) } };
    const effectiveStatus = trip_status || status;
    if (effectiveStatus) rideFilter.status = String(effectiveStatus).trim().toLowerCase();
    const dateFilter = buildDateFilter(date_option, from_date, to_date);
    if (dateFilter) rideFilter.createdAt = dateFilter;

    const rides = drivers.length
      ? await Ride.find(rideFilter).select('driverId fare commissionAmount driverEarnings').lean()
      : [];

    const totals = new Map();
    for (const ride of rides) {
      const ownerId = ownerByDriver.get(String(ride.driverId));
      if (!ownerId) continue;
      const acc = totals.get(ownerId) || { trips: 0, gross: 0, commission: 0, earnings: 0 };
      acc.trips += 1;
      acc.gross += Number(ride.fare || 0);
      acc.commission += Number(ride.commissionAmount || 0);
      acc.earnings += Number(ride.driverEarnings || 0);
      totals.set(ownerId, acc);
    }
    const money = (value) => Math.round(Number(value || 0) * 100) / 100;

    return {
      headers: ['company_name', 'owner', 'transport_type', 'active', 'trips', 'gross_fare', 'admin_commission', 'driver_earnings'],
      rows: owners.map((item) => {
        const acc = totals.get(String(item._id)) || { trips: 0, gross: 0, commission: 0, earnings: 0 };
        return {
          company_name: item.company_name,
          owner: item.name,
          transport_type: item.transport_type,
          active: item.active,
          trips: acc.trips,
          gross_fare: money(acc.gross),
          admin_commission: money(acc.commission),
          driver_earnings: money(acc.earnings),
        };
      }),
    };
  };
  ...
  const sanitizeGeneralSettings = (generalObj = {}) => {
    const rawAppName = String(generalObj.app_name || '').trim();
    if (!rawAppName || /appzeto|redigo|rydon/i.test(rawAppName)) {
      return { ...generalObj, app_name: 'GoKab' };
    }
    return generalObj;
  };
  ...
  const getGeneralBusinessSettingsSection = async () => {
    ...
    if (results.length === 0) {
      try {
        ...
        return sanitizeGeneralSettings(created.general || {});
      } catch (error) {
        if (error?.code !== 11000) {
          throw error;
        }
        // Another concurrent call already created the default document; fetch it instead.
        results = await AdminBusinessSetting.aggregate([
          { $match: { scope: 'default' } },
          { $project: generalBusinessSettingsProjection },
          { $limit: 1 },
        ]);
    ...
    return sanitizeGeneralSettings(results[0]?.general || {});
  };
